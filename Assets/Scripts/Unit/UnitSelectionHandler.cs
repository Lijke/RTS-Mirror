...
public class UnitSelectionHandler : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        Unit.AuthorityOnUnitDespawned += AuthorityHandleUnityDespawn;
    ...
    private void StartSelectionArea()
    {
        ...
        {
            foreach (Unit selectedUnit in SelectedUnit)
            {
    ...
    private void ClearSelectionArea()
    {
        ...
        foreach (Unit unit1 in player.GetMyunit())
        {
            if (SelectedUnit.Contains(unit1)) { continue; }
            Vector3 screenPosition = mainCamera.WorldToScreenPoint(unit1.transform.position);
            if(screenPosition.x > min.x && screenPosition.x < max.x  && screenPosition.y > min.y && screenPosition.y < max.y)
            {
                SelectedUnit.Add(unit1);
                unit1.Select();
            }
        }
    ...
    private void AuthorityHandleUnityDespawn(Unit unit)
    {
